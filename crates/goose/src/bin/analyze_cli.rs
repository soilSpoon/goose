...
//! Usage: cargo run -p goose --features code-analysis --bin analyze_cli -- <path> [--focus <symbol>] [--depth <n>] [--follow <n>] [--force]

fn main() {
    #[cfg(not(feature = "code-analysis"))]
    {
        eprintln!("Error: analyze_cli requires the `code-analysis` feature.");
        eprintln!("Run with: cargo run -p goose --features code-analysis --bin analyze_cli");
        std::process::exit(1);
    }

    #[cfg(feature = "code-analysis")]
    analyze_cli_main::run();
}

#[cfg(feature = "code-analysis")]
mod analyze_cli_main {
    use clap::Parser;
